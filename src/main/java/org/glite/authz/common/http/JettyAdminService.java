...
public class JettyAdminService {
  ...
  protected Server buildAdminService() {
    ...
    HttpConnectionFactory cf = new HttpConnectionFactory(config);

    ServerConnector connector = new ServerConnector(adminService, cf);

    connector.setHost(adminHost);
    connector.setPort(adminPort);
    adminService.addConnector(connector);
